...
module.exports = {
  login: function (req, res) {
    ...
    User.findOneByEmail( req.body.email, function(err, user) {
      ...
      user.isPasswordValid( req.body.password, function (err, bool) {
        ...
        if ( bool ) {
          return res.json( { user: {id: user.id, email: user.email, role: user.role}, token: TokenAuth.issueToken({ sub: user.id }) } );
        } else {
          return res.json( 401, { err: {
            status: 'danger',
  ...
  signup: function (req, res) {
    ...
    /***** BMO EDIT: Add all params that match extended attributes *****/
    var atts = _.keys(_.omit(User._attributes,['id','createdAt','updatedAt'])); // Skip these, they get autogenerated
    var newUser = {}; // Start with blank object
    _.forIn(atts,function(v,k){ // Loop through attributes
        var o = {};
        o[v] = req.body[v]; // Build temp object for this att
        newUser = _.assign(o,newUser); // Updated main object
    });
    ...
    bcrypt.genSalt(10, function(err, salt) {
      bcrypt.hash(newUser.password, salt, function(err, hash) {
        ...
        User.create(newUser).exec(function(err, user) {
          ...
          if ( user.active ) {
            return res.json( { user: user.id, token: TokenAuth.issueToken({ sub: user.id }) } );
          }
